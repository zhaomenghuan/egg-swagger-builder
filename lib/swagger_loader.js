...
module.exports = app => {
  ...
  swagger.basePath = app.config.swaggerbuilder.basePath;
  swagger.info = app.config.swaggerbuilder.apiInfo;
  swagger.schemes = app.config.swaggerbuilder.schemes;
  swagger.tags = [];
  swagger.paths = {};
  let securitys = [];
  let tagNames = [];
  if (app.config.swaggerbuilder.enableSecurity) {
    swagger.securityDefinitions = app.config.swaggerbuilder.securityDefinitions;
  ...
  const scannerDir = path.join(app.config.baseDir, app.config.swaggerbuilder.dirScanner);
      ...
      if (stat.isFile() && path.extname(name) === '.js') {
        ...
        for (let i = 1; i < blocks.length; i++) {
          ...
          if (routers) {
            ...
            path_method.consumes = app.config.swaggerbuilder.consumes;
            path_method.produces = app.config.swaggerbuilder.produces;
